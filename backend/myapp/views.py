...
class UserCreateView(CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if User.objects.filter(username=request.data["username"]).count() >0:
            return JsonResponse({"message":"non-correct username"}, status=status.HTTP_400_BAD_REQUEST)
...
def PostGetView(request):
    try:
        ...
        for resource_relation_id in ResourcesRelation.objects.filter(post_id=post.id):
            resources.append(ResourcesData.objects.filter(resource_id=resource_relation_id).resource_url)
...
def auth_by_hash(request):
    ...
    if not User.objects.filter(username=name, password_hash=passhash).count() > 0:
        return JsonResponse({"message":"User is not exist"}, status=status.HTTP_400_BAD_REQUEST)
